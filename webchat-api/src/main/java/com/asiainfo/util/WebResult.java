package com.asiainfo.util;

public class WebResult {
	public static final String APPLY_ACCOUNT_FAIL="很遗憾,账号申请失败,请稍后重试!";
	public static final String SEND_EMAIL_FAIL="很遗憾,邮件发送失败,申请未能成功,请稍后重试!";
	public static final String APPLY_ACCOUNT_SUCCESS="恭喜您,申请成功,邮件已发送至您的邮箱,请查收!";
	
	public static final String ACTIVE_ACCOUNT_FAIL="很遗憾,账号激活失败,请稍后重试!";
	public static final String ACTIVE_ACCOUNT_SUCCESS="恭喜您,账号激活成功,请使用您的账号登录系统开始您的聊天之旅吧!";
	
	public static final String Email_UNAVAILABLE="该邮箱已经被注册过了！";
	
	public static final String Account_Activated="该账号已激活，无须再次激活！";
	
	public static final String USER_NOT_EXIST="该用户不存在！";
	
	public static final String LOGIN_FAIL="登录失败，请稍后重试！";
	
	public static final String PASSWORD_ERROR="密码不正确，请输入正确的密码！";
	
	public static final String LOGIN_SUCCESS="登录成功！";
	
	public static final String INIT_USER_ROOT_SUCCESS="初始化用户根路径成功！";
	
	public static final String INIT_USER_ROOT_FAIL="初始化用户根路径失败！";
}
